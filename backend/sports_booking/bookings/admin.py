...
class BookingAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Cancellation', {
            'fields': ('cancelled_at', 'cancellation_reason', 'admin_notes'),
            'classes': ('collapse',)
        }),
        ('Timestamps', {
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('user', 'club', 'sport', 'lock')
...
class SlotWaitlistAdmin(admin.ModelAdmin):
    list_display = ['id', 'user', 'club', 'sport', 'date', 'start_time', 'get_status', 'created_at']
    ...
    def get_status(self, obj):
        return '✅ Notified' if obj.notified else '⏳ Waiting'
    ...
    def send_notification_now(self, request, queryset):
        ...
        count = 0
        # Dedupe by (club, sport, date, start_time, end_time) — the task
        # already notifies every waitlisted user for that slot in one go,
        # so calling it once per unique slot avoids sending duplicate emails.
        seen_slots = set()
        for entry in queryset.filter(notified=False):
            slot_key = (entry.club_id, entry.sport_id, entry.date, entry.start_time, entry.end_time)
            ...
            count += 1
        self.message_user(request, f'Notification queued for {count} slot(s).')
